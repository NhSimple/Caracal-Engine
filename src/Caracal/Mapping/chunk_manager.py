...
class ChunkManager:
    ...
    def load_chunk(self, chunkpos):
        with open(self.world_root_dir + "/chunks/"+f"{chunkpos}.chunk",
                    "r") as chunkfile:
    ...
    def draw(self, surf):
        for x in range(self.centerpos - (self.CHUNK_WIDTH//2),
                       self.centerpos + (self.CHUNK_WIDTH//2)):
            try:
                surf.blit(self.loaded_chunks[x]["image"],
                      (x*self.CHUNK_WIDTH*self.BLOCK_PIXEL_SIZE - self.camera.pos[0],
                       -self.camera.pos[1]))
                ...
                draw.rect(surf, "black", (xpos, ypos, self.CHUNK_WIDTH*self.BLOCK_PIXEL_SIZE, self.CHUNK_HEIGHT*self.BLOCK_PIXEL_SIZE))

    def set_centerpos(self, new_x: int):
        self.calc_centerpos(new_x)

    def calc_centerpos(self, new_x):
        self.centerpos = new_x//(self.CHUNK_WIDTH*self.BLOCK_PIXEL_SIZE)  # update pos
